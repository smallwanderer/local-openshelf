...
def test_parse_docling_document_returns_parse_result(mock_dependencies):
    ...
    assert dto.parser_backend == "docling"
    ...
    meta = dto.backend_metadata
    assert meta["timestamp"] == "2026-03-26T10:00:00"
    assert meta["input_file"] == "/tmp/sample.pdf"
    assert meta["input_filesize"] == 12345
    assert meta["docling_core_version"] == "1.1.0"
    assert meta["docling_ibm_models_version"] == "1.2.0"
    assert meta["docling_parse_version"] == "1.3.0"
    assert meta["platform_str"] == "linux-x86_64"
    assert meta["py_impl_version"] == "cpython"
    assert meta["py_lang_version"] == "3.12"
    assert meta["document"] == {}
    assert meta["pages"] == [{"page_no": 1}, {"page_no": 2}]
    assert meta["confidence"] == {"score": 0.95}
    assert meta["assembled"] == {"body": True}
...
def test_parse_docling_document_dump_json(mock_dependencies):
    ...
    assert "backend_metadata" in dumped
...
def test_parse_docling_document_missing_fields(mock_dependencies):
    ...
    assert meta["input_info"] is None
    assert meta["version_info"] is None
    assert meta["pages"] is None
    assert meta["confidence"] is None
    assert meta["assembled"] is None
