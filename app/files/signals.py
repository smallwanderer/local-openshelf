...
def create_workspace_storage(sender, instance, created, **kwargs):
    """Create the storage quota record owned by each workspace."""
    if created:
        UserStorage.objects.get_or_create(
            workspace=instance,
            defaults={"user": instance.created_by},
        )
...
def update_used_size_on_save(sender, instance, created, **kwargs):
    ...
    if created and instance.size:
        UserStorage.objects.filter(workspace=instance.node.workspace).update(
