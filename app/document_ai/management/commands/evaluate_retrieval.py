...
def _rank_documents(
    ...
    chunk_hits = []
    for record, vector in zip(chunk_records, chunk_vectors):
    ...
    chunk_hits.sort(key=lambda item: item["distance"])
    ...
    for node_id, hits in grouped_hits.items():
        ...
        top_hits = hits[:3]
        combined_score = sum(1.0 / (1.0 + item["distance"]) for item in top_hits)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        backends = options.get("backends") or [get_embedding_backend(), "hf_cls_legacy"]
